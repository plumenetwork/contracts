...
import { OptionsHelper } from "@layerzerolabs/lz-evm-oapp-v2/test/OptionsHelper.sol";
...
import "@layerzerolabs/lz-evm-oapp-v2/test/mocks/ExecutorFeeLibMock.sol";

// file copied and modified from original package to avoid problems with stack too deep
        ...
        for (uint8 i = 0; i < _endpointNum; i++) {
            if (_libraryType == LibraryType.UltraLightNode) {
                ...
                
                {
                    ...
                    libSetup.executor.setWorkerFeeLib(address(executorLib));
                    ...
                    libSetup.dvn.setWorkerFeeLib(address(dvnLib));
                }
                ...
                for (uint8 j = 0; j < _endpointNum; j++) {
...

                    {
                        ...
                        libSetup.sendUln.setDefaultUlnConfigs(params);
                    }

                    {
                        SetDefaultExecutorConfigParam[] memory params = new SetDefaultExecutorConfigParam[](1);
                        ExecutorConfig memory executorConfig = ExecutorConfig(10000, address(libSetup.executor));
                        params[0] = SetDefaultExecutorConfigParam(dstEid, executorConfig);
                        libSetup.sendUln.setDefaultExecutorConfigs(params);
                    }

                    {
                        ...
                        params[0] = SetDefaultUlnConfigParam(dstEid, ulnConfig);
                        libSetup.receiveUln.setDefaultUlnConfigs(params);
                    }
