...
contract MockComponentToken is Initializable, ERC20Upgradeable, IComponentToken {
    ...
    function requestDeposit(uint256 assets, address controller, address owner) external override returns (uint256) {
        if (assets == 0) {
            revert ZeroAmount();
        }
        if (msg.sender != owner) {
    ...
    function requestRedeem(uint256 shares, address controller, address owner) external override returns (uint256) {
        if (shares == 0) {
            revert ZeroAmount();
        }
        if (msg.sender != owner) {
    ...
    function deposit(uint256 assets, address receiver, address controller) external override returns (uint256) {
        return assets;
    }

    function redeem(uint256 shares, address receiver, address controller) external override returns (uint256) {
        return shares;
    }
    ...
    function claimableDepositRequest(uint256, address controller) external view override returns (uint256) {
    ...
    function supportsInterface(
        ...
        return interfaceId == type(IERC20).interfaceId || interfaceId == type(IAccessControl).interfaceId
            || interfaceId == type(IERC7575).interfaceId || interfaceId == type(IComponentToken).interfaceId
